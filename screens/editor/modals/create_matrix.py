"""
Modal: Crear Matriz
Permite crear variables de tipo matriz con soporte de magnitudes y unidades.
"""
...
async def open_create_matrix_modal(
    page,
    pool,
    columns_row,
    ...
    name_field = text_input(
        placeholder=tm.translate("Nombre variable (ej: M1, A...)")
    )
    ...
    # ── Spinners de dimensiones ───────────────────────────────────────────────
    rows_value = [3]
    cols_value = [3]

    rows_display = ft.Text(
        str(rows_value[0]),
        size=20,
        weight="bold",
        text_align=ft.TextAlign.CENTER,
    )
    cols_display = ft.Text(
        str(cols_value[0]),
        size=20,
        weight="bold",
        text_align=ft.TextAlign.CENTER,
    ...
    def _update_rows_display():
        rows_display.value = str(rows_value[0])
        try:
            rows_display.update()
        except:
            pass

    def _update_cols_display():
        cols_display.value = str(cols_value[0])
        try:
            cols_display.update()
        except:
            pass

    def _increment_rows(e):
        rows_value[0] = min(rows_value[0] + 1, 10)
        _update_rows_display()

    def _decrement_rows(e):
        rows_value[0] = max(rows_value[0] - 1, 1)
        _update_rows_display()

    def _increment_cols(e):
        cols_value[0] = min(cols_value[0] + 1, 10)
        _update_cols_display()

    def _decrement_cols(e):
        cols_value[0] = max(cols_value[0] - 1, 1)
        _update_cols_display()

    rows_spinner = ft.Row(
        [
            ft.IconButton(ft.Icons.REMOVE, on_click=_decrement_rows, icon_size=16),
            ft.Container(rows_display, width=40, alignment=ft.Alignment.CENTER),
            ft.IconButton(ft.Icons.ADD, on_click=_increment_rows, icon_size=16),
        ],
        spacing=0,
    ...
    cols_spinner = ft.Row(
        [
            ft.IconButton(ft.Icons.REMOVE, on_click=_decrement_cols, icon_size=16),
            ft.Container(cols_display, width=40, alignment=ft.Alignment.CENTER),
            ft.IconButton(ft.Icons.ADD, on_click=_increment_cols, icon_size=16),
        ],
        spacing=0,
        alignment=ft.MainAxisAlignment.CENTER,
    )

    dimensions_controls = ft.Column(
        [
            ft.Row(
                [
                    ft.Column([ft.Text(tm.translate("Filas"), size=12, color=_c(0.6)), rows_spinner], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                    ft.VerticalDivider(width=20),
                    ft.Column([ft.Text(tm.translate("Columnas"), size=12, color=_c(0.6)), cols_spinner], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
            )
        ],
    ...
    name_field.on_blur = lambda e: _on_name_blur()
    ...
    step0 = ft.Container(
        content=ft.Column(
            [
                ...
                _card(desc_field),
                _section_header(tm.translate("Dimensiones de la matriz")),
                _card(dimensions_controls),
            ],
            spacing=0,
            expand=True,
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    ...
    step1 = ft.Container(
        content=ft.Column(
            [
                ...
                _card(ft.Row([mag_dropdown], alignment=ft.MainAxisAlignment.CENTER)),
    ...
    back_btn = text_btn(tm.translate("← Atrás"), on_click=lambda e: _go_step0(e))
    back_btn.visible = False

    next_btn = filled_btn(tm.translate("Siguiente →"), on_click=lambda e: _go_step1(e))
    create_btn = filled_btn(tm.translate("Crear matriz"), on_click=lambda e: _save(e))
    ...
    def _go_step1(e):
        ...
        _refresh_dots()
        try:
            step0.update(); step1.update(); back_btn.update(); next_btn.update(); create_btn.update()
        except:
            pass
    ...
    def _save(e):
        ...
        # Una matriz por variable: values es una lista que contiene UNA matriz
        # La matriz es una lista de listas: [ [0,0,..], [0,0,..], ... ]
        # Para que sea compatible con el sistema de columnas que itera sobre 'values',
        # lo ponemos como [ matriz ].
        
        matrix_data = [["0" for _ in range(c)] for _ in range(r)]
        
        pool[var_name] = {
            "values": [matrix_data],
            "errors": [],
            "type": VARIABLE_TYPE_MATRIX,
    ...
    page.show_dialog(
        modal(
            title_str=tm.translate("Crear Matriz"),
