...
def resolve_node(
    ...
    if isinstance(node, FuncNode):
        ...
        return ResolvedNode(
            node=node,
            children=[resolve_node(child, variables, extra_constants, operations) for child in node.args],
            operation=op,
        )
    if isinstance(node, AddNode):
        return ResolvedNode(node=node, children=[resolve_node(child, variables, extra_constants, operations) for child in node.operands])
    if isinstance(node, MulNode):
        return ResolvedNode(node=node, children=[resolve_node(child, variables, extra_constants, operations) for child in node.operands])
    if isinstance(node, PowNode):
        return ResolvedNode(node=node, children=[resolve_node(node.base, variables, extra_constants, operations), resolve_node(node.exponent, variables, extra_constants, operations)])
    if isinstance(node, CompareNode):
        return ResolvedNode(node=node, children=[resolve_node(node.left, variables, extra_constants, operations), resolve_node(node.right, variables, extra_constants, operations)])
